...
export async function GET(req: NextRequest) {
  ...
  // 該当アセットがパックに含まれているか確認
  if (!pack.assetIds.includes(Number(assetId))) {
    return NextResponse.json({ error: 'Asset not in this pack' }, { status: 403 })
  }
